...
namespace LinqToEdmx.Model.Storage
{
  ...
  public class EntityProperty : XTypedElement, IXMetaData
  {
    ...
    public string StoreGeneratedPattern
    {
      get
      {
        var x = Attribute(XName.Get("StoreGeneratedPattern", ""));
        return XTypedServices.ParseValue<string>(x, XmlSchemaType.GetBuiltInSimpleType(XmlTypeCode.Token).Datatype);
      }
      set
      {
        SetAttribute(XName.Get("StoreGeneratedPattern", ""), value, XmlSchemaType.GetBuiltInSimpleType(XmlTypeCode.Token).Datatype);
      }
    }
    ...
    XName IXMetaData.SchemaName
    {
      get
      {
        return XName.Get("TEntityProperty", XMLNamespaceFactory.SSDL);
      }
    }
