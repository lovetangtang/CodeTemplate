...
{
    public class SQLAnywhereSchemaProvider : IDbSchemaProvider, IDbConnectionStringEditor
    {
        #region Member Variables
        private string _databaseName = String.Empty;
        private string _saServerVersion;
        private int _saServerMajorVersion;
        private bool _saServerMajorVersionChecked;
        #endregion
        ...
        public string GetDatabaseName(string connectionString)
        {
            ...
            {
                SQLAnywhereService sa = new SQLAnywhereService(connectionString) {IsSingleRow = true};
                using (SADataReader reader = sa.ExecuteSAReader(SQL_GetDatabaseName))
                {
                    while (reader.Read())
                    {
        ...
        public TableSchema[] GetTables(string connectionString, DatabaseSchema database)
        {
            ...
            sqlstring.AppendStatement(SqlFactory.GetTables(majorVersion));

            if (database.DeepLoad)
            {
                sqlstring.AppendStatement(SqlFactory.GetAllTableColumns(majorVersion));
                sqlstring.AppendStatement(SqlFactory.GetIndexes(majorVersion));
            ...
            SQLAnywhereService sql = new SQLAnywhereService(connectionString);
        ...
        public ColumnSchema[] GetTableColumns(string connectionString, TableSchema table)
        {
            ...
            SQLAnywhereService sql = new SQLAnywhereService(connectionString);
            sql.AddParameter(null, SADbType.NVarChar, table.Name);
            sql.AddParameter(null, SADbType.NVarChar, table.Owner);
        ...
        private List<ColumnSchema> GetColumnsFromReader(IDictionary<string, TableSchema> tables, SafeDataReader reader, bool populateTable)
        {
            ...
            while (reader.Read())
            {
                ...
                DbType dataType = Utility.GetDbType(systemType, precision, scale);
        ...
        private void PopulateTableExtendedData(SafeDataReader reader, IList<TableSchema> tables)
        {
            ...
            while (reader.Read())
            {
                ...
                if (objectType == "U")
                {
                    ...
                    {
                        // column id = 0 means we are dealing with an object other than a column or proc parameter
        ...
        public ViewSchema[] GetViews(string connectionString, DatabaseSchema database)
        {
            ...
            sqlstring.AppendStatement(SqlFactory.GetViews(majorVersion));
            if (database.DeepLoad)
            {
        ...
        public ViewColumnSchema[] GetViewColumns(string connectionString, ViewSchema view)
        {
            ...
            SQLAnywhereService sql = new SQLAnywhereService(connectionString);
            sql.AddParameter(null, SADbType.NVarChar, view.Name);
            sql.AddParameter(null, SADbType.NVarChar, view.Owner);
        ...
        public IndexSchema[] GetTableIndexes(string connectionString, TableSchema table)
        {
            ...
            string sqlString = SqlScripts.GetTableIndexes;
            using (SafeDataReader reader = sql.ExecuteSafeReader(sqlString))
            {
        ...
        public TableKeySchema[] GetTableKeys(string connectionString, TableSchema table)
        {
            ...
            string sqlString = SqlScripts.GetTableKeys;
            using (SafeDataReader reader = sql.ExecuteSafeReader(sqlString))
            {
        ...
        public string GetViewText(string connectionString, ViewSchema view)
        {
            ...
            SQLAnywhereService sql = new SQLAnywhereService(connectionString);
            sql.AddParameter(null, SADbType.Char, view.Owner + "." + view.Name);

            using (SADataReader reader = sql.ExecuteSAReader(SQL_GetObjectSource))
            {
                while (reader.Read())
                {
                  viewText.AppendLine(reader.GetString(0));
                }
            }
        ...
        public CommandSchema[] GetCommands(string connectionString, DatabaseSchema database)
        {
            ...
            sqlstring.AppendStatement(SqlFactory.GetCommands(majorVersion));
            if (database.DeepLoad)
            {
        ...
        public ParameterSchema[] GetCommandParameters(string connectionString, CommandSchema command)
        {
            ...
            SQLAnywhereService sql = new SQLAnywhereService(connectionString);
            sql.AddParameter(null, SADbType.NVarChar, command.Name);
            sql.AddParameter(null, SADbType.NVarChar, command.Owner);
        ...
        private List<ParameterSchema> GetParametersFromReader(IDictionary<string, CommandSchema> commands, SafeDataReader reader, bool populateCommand)
        {
            ...
            while (reader.Read())
            {
                ...
                DbType dataType = string.IsNullOrEmpty(nativeType) ? DbType.Object : Utility.GetDbType(nativeType, precision, scale);
                string userDefinedType = reader.GetString("TypeName");
                DbType userDefinedDataType = string.IsNullOrEmpty(userDefinedType) ? DbType.Object : Utility.GetDbType(userDefinedType, precision, scale);
                int size = reader.GetInt32("Length");
        ...
        public string GetCommandText(string connectionString, CommandSchema command)
        {
            ...
            if (!Utility.ParseBooleanExtendedProperty(command, ExtendedPropertyNames.IsCLR))
            {
                ...
                SQLAnywhereService sql = new SQLAnywhereService(connectionString);
                sql.AddParameter(null, SADbType.Char, command.Owner + "." + command.Name); 

                using (SADataReader reader = sql.ExecuteSAReader(SQL_GetObjectSource))
                {
                    while (reader.Read())
                    {
                        commandText.AppendLine(reader.GetString(0));
                    }
                }
        ...
        public CommandResultSchema[] GetCommandResultSchemas(string connectionString, CommandSchema command)
        {
            ...
            int majorVersion = GetSqlServerMajorVersion(connectionString);
            //string sqlstring = String.Format(SqlFactory.GetCommandResultSchema(majorVersion), command.Owner, command.Name);

            string sqlstring = SqlFactory.GetCommandResultSchema(majorVersion);

            sql.AddParameter(null, SADbType.NVarChar, command.Name);
            sql.AddParameter(null, SADbType.NVarChar, command.Owner);

            reader = sql.ExecuteSAReader(sqlstring);
        ...
        private static List<CommandResultSchema> CommandResultSchemaHelper(CommandSchema command, SADataReader reader)
        {
            List<CommandResultSchema> results = new List<CommandResultSchema>();
            ...
            if (reader.HasRows)  
            {
              while (reader.Read())
	         {
                ...
                SADbType sqlDbType = Utility.GetSqlDbType(reader["baseTypeName"].ToString());
                string nativeType = Utility.GetNativeType(sqlDbType);
                int size = (int) (long) reader["Length"];
                byte precision = (byte) (long) reader["NumericPrecision"];
                int scale = (int) (short) reader["NumericScale"];
                DbType dataType = Utility.GetDbType(nativeType, precision, scale);
                bool allowDBNull = (bool) (reader["AllowDBNull"].ToString() == "Y");
            ...
            CommandResultSchema result = new CommandResultSchema(command, "1", columns.ToArray());
        ...
        private ExtendedPropertyCollection GetColumnExtendedProperties(string connectionString, ColumnSchema column)
        {
            ...
            SQLAnywhereService sql = new SQLAnywhereService(connectionString);
            sql.AddParameter(null, SADbType.NVarChar, column.Table.Owner);
            sql.AddParameter(null, SADbType.NVarChar, column.Table.Name);
            sql.AddParameter(null, SADbType.NVarChar, column.Name);
        ...
        private ExtendedProperty[] GetExtendedProperties(string connectionString, string level0type, string level0name, string level1type, string level1name, string level2type, string level2name)
        {
            var extendedProperties = new Dictionary<string, ExtendedProperty>(StringComparer.OrdinalIgnoreCase);
              // Not needed for SQLAnywhere, but required to preserve interface
            //string description = String.Empty;

            //SQLAnywhereService sql = new SQLAnywhereService(connectionString);
            //sql.AddParameter("@level0type", SADbType.VarChar, level0type, true);
            //sql.AddParameter("@level0name", SADbType.VarChar, level0name, true);
            //sql.AddParameter("@level1type", SADbType.VarChar, level1type, true);
            //sql.AddParameter("@level1name", SADbType.VarChar, level1name, true);
            //sql.AddParameter("@level2type", SADbType.VarChar, level2type, true);
            //sql.AddParameter("@level2name", SADbType.VarChar, level2name, true);
        ...
        private void SetExtendedProperties(
            ...
            string level2name)
        {
            SQLAnywhereService sql = null;

            try
            {
                bool needRefresh = false;
                sql = new SQLAnywhereService(connectionString);
                ...
                foreach (ExtendedProperty p in schemaObject.ExtendedProperties)
                {
                    ...
                    {
                        sql.Reset();
                        sql.AddParameter("@name", SADbType.VarChar, name, true);
        ...
        {
            if (_saServerVersion == null)
            {
                _saServerVersion = String.Empty;
                try
                {
                    SQLAnywhereService sql = new SQLAnywhereService(connectionString);
                    DataTable schemaTable = sql.ExecuteSqlDataSet(SQL_GetSqlServerVersion).Tables[0];

                    _saServerVersion = (string)schemaTable.Rows[0][0];
                }
                catch { }
            }

            return _saServerVersion;
        }

        private int GetSqlServerMajorVersion(string connectionString)
        {
            if (!_saServerMajorVersionChecked)
            {
                _saServerMajorVersionChecked = true;
                string version = GetSqlServerVersion(connectionString);
                if (version.Length > 0)
                {
                    ...
                    if (index > 0)
                    {
                        _saServerMajorVersion = Int32.Parse(version.Substring(0, index));
                    }
                }
            }

            return _saServerMajorVersion;
        }
        ...
        private const string SQL_GetObjectSource = "EXEC sp_helptext ? ";

        private const string SQL_GetSqlServerVersion = "SELECT PROPERTY ('ProductVersion')";
